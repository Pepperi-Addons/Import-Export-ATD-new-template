...
async function doExport(
    ...
    await Promise.all(getDataPromises).then(async (result1) => {
        ...
        getReferencesPromises.push(
            getDataViewReferences(references, dataViews)
        );
        getReferencesPromises.push(
    ...
    fields.forEach(async (field) => {
        if (field.TypeSpecificFields != null) {
            if (field.TypeSpecificFields.ReferenceToResourceType != null) {
                const referenceId =
                    field.TypeSpecificFields.ReferenceToResourceType.ID;
            ...
            if (field.UserDefinedTableSource != null) {
                ...
                const udt: UserDefinedTableMetaData = JSON.parse(
                    JSON.stringify(udts)
                );
                const reference: Reference = {
...
async function upsertDataViews(
    ...
    try {
        ...
        console.debug(
            `posting data views: ${JSON.stringify({ hack: dataViews })}`
        );
    ...
    try {
        fields.forEach(async (field) => {
            if (field.TypeSpecificFields != null) {
                if (field.TypeSpecificFields != null) {
                    if (
                        field.TypeSpecificFields.ReferenceToResourceType != null
                    ) {
                        const referenceUUID =
                            field.TypeSpecificFields.ReferenceToResourceType
                                .UUID;
                ...
                if (field.UserDefinedTableSource != null) {
                    const pairIndex = map.Mapping.findIndex(
                        (x) =>
                            x.Origin.ID === field.UserDefinedTableSource.TableID
                    );
            ...
            switch (ref.Type) {
                ...
                case ReferenceType.FileStorage:
                    const referenceDataFileNameIndex = referencesDataList.findIndex(
                        (data) =>
                            data.Title && data.Title.toString() === ref.Name
                    );
                ...
                case ReferenceType.Catalog:
                    const referenceDataExternalIDIndex = referencesDataList.findIndex(
                        (data) =>
                            data.ExternalID &&
                            data.ExternalID.toString() === ref.Name
                    );
                ...
                case ReferenceType.UserDefinedTable:
                    const referenceDataTableIDIndex = referencesDataList.findIndex(
                        (data) =>
                            data.TableID && data.TableID.toString() === ref.Name
                    );
    ...
    exportReferences.forEach((ref) => {
        if (ref.Type !== ReferenceType.Webhook) {
            ...
            const referenceDataIdIndex = referencesDataList.findIndex(
                (data) =>
                    data.InternalID &&
                    data.InternalID.toString() === ref.ID.toString()
            );
